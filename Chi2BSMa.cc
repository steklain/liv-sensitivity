...
{ 
  ...
  */
  ...
  for(a=ai;a<af;a=a+(af-ai)/points)
  {
      ...
      glbSetOscParams(test_values,49.2*M_PI/180,GLB_THETA_23);
      glbSetOscParams(test_values,331.46*M_PI/180,GLB_DELTA_CP);
      glbSetOscParams(test_values,4.02124,GLB_ARG_AEMU); 
      resmin=glbChiNP(test_values,minimum,GLB_ALL);
    
      glbSetOscParams(test_values,42.052*M_PI/180,GLB_THETA_23);
      glbSetOscParams(test_values,207.86*M_PI/180,GLB_DELTA_CP);
      glbSetOscParams(test_values,1.31947,GLB_ARG_AEMU); 
      res=glbChiNP(test_values,minimum,GLB_ALL);

      if(resmin<res) res=resmin;
